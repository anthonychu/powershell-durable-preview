...
1. When the function app has started, a URL for the HTTP triggered function is printed in the terminal (you may have to scroll up to find it).
    ...
    > In VS Online in the browser, the URL may not work. Click on the *Remote Explorer* icon in the VS Code activity bar. Under *Environment Details*, port *7071* should already be listed as forwarded. Right-click on it and select *Copy Port URL*. Use this as the base URL instead of `http://localhost:7071`.
